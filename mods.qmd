...
mod_total_team_svm = readRDS("mod_total_team_svm_2025_12_08.rds")
  ...
  mutate(
    ...
    rolling_off_rush_epa =
      case_when(
        ...
        .default = 0.75 * rolling_sum_recent_off_rush_epa_recent + 0.25 * rolling_sum_prev_off_rush_epa_prev
      ),
    rolling_off_pass_epa =
      case_when(
        n_game <= 4 ~ sum_off_pass_epa / n_game,
        .default = 0.75 * rolling_sum_recent_off_pass_epa_recent + 0.25 * rolling_sum_prev_off_pass_epa_prev
      ),
    rolling_def_rush_epa =
      case_when(
        n_game <= 4 ~ sum_def_rush_epa / n_game,
        .default = 0.75 * rolling_sum_recent_def_rush_epa_recent + 0.25 * rolling_sum_prev_def_rush_epa_prev
      ),
    rolling_def_pass_epa =
      case_when(
        n_game <= 4 ~ sum_def_pass_epa / n_game,
        .default = 0.75 * rolling_sum_recent_def_pass_epa_recent + 0.25 * rolling_sum_prev_def_pass_epa_prev
      ),
    across(starts_with("rolling_"), ~ lag(.), .names = "prior_{col}")
  ) %>%
  ...
  mutate(
    ...
    rolling_pace =
      case_when(
        ...
        .default = 0.75 * rolling_recent + 0.25 * rolling_prev
      ),
    across(starts_with("rolling_"), ~ lag(.), .names = "prior_{col}")#,
    # rolling_pace = cumsum(n_plays) / n_game
  ) %>%
  ...
  mutate(
    ...
    rolling_success_off_rate =
      case_when(
        ...
        .default = 0.75 * rolling_recent_off + 0.25 * rolling_prev_off
      ),
    rolling_success_def_rate =
      case_when(
        n_game <= 4 ~ (sum_sum_success_def / sum_n_success_def),
        .default = 0.75 * rolling_recent_def + 0.25 * rolling_prev_def
      ),
    across(starts_with("rolling_") & ends_with("_rate"), ~ lag(.), .names = "prior_{col}")
  ) %>%
  select(-starts_with("sum_"), -ends_with("_prev"), -ends_with("_recent")) %>%
...
df_pred_total_spread_fin %>% 
  ...
  mutate(
    ...
    bet_spread = 
      case_when(
        ind_spread_current_hit == 1 & ind_spread_hit == 1 & spread_game_adj > spread_line_current & spread_line_current < 0 & spread_line < 0 ~ str_c(team_home, " +", -spread_line_current),
        ind_spread_current_hit == 1 & ind_spread_hit == 1 & spread_game_adj > spread_line_current & spread_line_current >= 0 & spread_line >= 0 ~ str_c(team_home, " ", -spread_line_current),
        ind_spread_current_hit == 1 & ind_spread_hit == 1 & spread_game_adj < spread_line_current & spread_line_current < 0 & spread_line < 0 ~ str_c(team_away, " ", spread_line_current),
        ind_spread_current_hit == 1 & ind_spread_hit == 1 & spread_game_adj < spread_line_current & spread_line_current >= 0 & spread_line >= 0 ~ str_c(team_away, " +", spread_line_current),
        .default = "NA"
      ),
...
df_totals_hit_rate =
  ...
  # filter(ind_total_hit != "No Bet" | ind_total_hit_close != "No Bet") %>% 
  mutate(
    ind_bet_hit =
      case_when(
...
df_spreads_hit_rate =
  ...
  # filter(ind_spread_hit != "No Bet" | ind_spread_hit_close != "No Bet") %>% 
  mutate(
    ind_bet_hit =
      case_when(
...
df_team_ranks =
  ...
  mutate(qb_cont = ifelse(team == "IND", 0, qb_cont)) %>% 
  left_join(
    df_total_team_test_solo %>% 
      select(team, starts_with("prior_rolling")) %>% 
      mutate(
