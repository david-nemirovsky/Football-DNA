...
df_team_sims =
  df_total_team_test %>% 
  filter(team %nin% c("NO", "ATL", "ARI", "LA")) %>% 
  mutate(
    game = 
      case_when(
...
df_team_sims_all = 
  ...
  left_join(df_team_total_spread_rolling %>% select(game_id, week, team, total_team), by = c("week", "team", "game_id")) %>%
  filter(!(week == 17 & team %in% c("ATL", "LA"))) %>% 
  mutate(
    mu = predict(mod_total_team_svm, .),
    sim = map(
      mu,
...
df_totals_profit =
  ...
  left_join(df_lines_open %>% distinct(game_id, total_line) %>% mutate(over_odds = -110, under_odds = -110), by = "game_id") %>% 
  mutate(
    p_over = map_dbl(total_sim_adj, ~ mean(.x > total_line)),
    p_under = 1 - p_over,
    p_over_current  = map_dbl(total_sim_adj, ~ mean(.x > total_line_current)),
...
df_spreads_profit =
  ...
  left_join(df_lines_open %>% distinct(game_id, spread_line) %>% mutate(home_odds = -110, away_odds = -110), by = "game_id") %>% 
  mutate(
    p_home = map_dbl(spread_sim_adj, ~ mean(.x > spread_line)),
    p_away = 1 - p_home,
    p_home_current = map_dbl(spread_sim_adj, ~ mean(.x > spread_line_current)),
