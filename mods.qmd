...
df_pred_total_spread_fin %>% 
  ...
  filter(ind_total_hit == 1 | ind_spread_hit) %>% 
  select(game_id, team_home, team_away, total_line, total_line_label, total_game_adj, ind_total_hit, spread_line, spread_line_label, spread_game_adj, ind_spread_hit, spread_band_adj_label) %>% 
  ...
  mutate(
    bet_total = 
      case_when(
        ind_total_hit == 1 & total_game_adj > total_line ~ str_c("Over ", total_line_label),
        ind_total_hit == 1 & total_game_adj < total_line ~ str_c("Under ", total_line_label),
        .default = "NA"
      ),
    bet_spread = 
      case_when(
        ind_spread_hit == 1 & spread_game_adj > spread_line & spread_line < 0 ~ str_c(team_home, " +", -spread_line),
        ind_spread_hit == 1 & spread_game_adj > spread_line & spread_line >= 0 ~ str_c(team_home, " ", -spread_line),
        ind_spread_hit == 1 & spread_game_adj < spread_line & spread_line < 0 ~ str_c(team_away, " ", spread_line),
        ind_spread_hit == 1 & spread_game_adj < spread_line & spread_line >= 0 ~ str_c(team_away, " +", spread_line),
        .default = "NA"
      ),
  ...
  select(game, total_mod_label, bet_total, spread_mod_label, bet_spread) %>% 
  ...
  gtreg::tbl_listing() %>% 
  gtsummary::modify_caption("**Bets to Take This Week**")
```
