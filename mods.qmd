...
#   )
# 
# saveRDS(df_total_team_pred_bands, "df_total_team_pred_bands.rds")
df_total_team_pred_bands = readRDS("df_total_team_pred_bands.rds")
...
#   )
# 
# saveRDS(df_total_team_pred_bands_prior, "df_total_team_pred_bands_prior.rds")
df_total_team_pred_bands_prior = readRDS("df_total_team_pred_bands_prior.rds")
...
df_pred_total_spread_fin %>% 
  ...
  mutate(
    ind_total_hit = 
      case_when(
        total_line > total_game_upper_adj + 0.5 ~ 1,
        total_line < total_game_lower_adj - 0.5 ~ 1,
        .default = 0
      ),
    ind_spread_hit = 
      case_when(
        spread_line > spread_upper_adj + 0.5 ~ 1,
        spread_line < spread_lower_adj - 0.5 ~ 1,
        .default = 0
      )
  ) %>% 
...
df_team_points_delta_szn = 
  ...
  mutate(team_points_diff_actual = team_points_mod - total_team) %>% 
  ungroup() %>% 
  group_by(team) %>% 
  mutate(
    ...
    points_text_label_actual = str_c("\U0394=", round(mean_team_points_diff_actual, 2)),
    points_text_label_position = 
      case_when(
    ...
    points_text_label = str_c("\U0394=", round(mean_team_points_diff, 2))
  ) %>% 
  ungroup() %>% 
  distinct(team, mean_team_points_diff, mean_team_points_diff_actual, points_text_label_position, points_text_label, points_text_label_position_actual, points_text_label_actual, team_color, team_logo_espn)
...
df_totals_hit_rate %>% 
  ...
  rows_insert(
    tibble(
      week = "Season",
      n_hit_under = sum(.$n_hit_under),
      n_hit_over = sum(.$n_hit_over),
      n_total_under = sum(.$n_total_under),
      n_total_over = sum(.$n_total_over)
    )
  ) %>% 
  mutate(
    hit_rate_under = 
...
df_spreads_hit_rate %>% 
  ...
  rows_insert(
    tibble(
      week = "Season",
      n_hit_home = sum(.$n_hit_home),
      n_hit_away = sum(.$n_hit_away),
      n_total_home = sum(.$n_total_home),
      n_total_away = sum(.$n_total_away)
    )
  ) %>% 
  mutate(
    hit_rate_home = 
