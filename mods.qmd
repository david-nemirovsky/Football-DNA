...
df_team_total_spread_rolling =
  ...
  left_join(df_qb_cont %>% select(-game_id), by = c("season", "week", "team")) %>% 
  left_join(df_rest_days %>% select(-game_id), by = c("season", "week", "team")) %>% 
  left_join(df_team_travel %>% select(-game_id), by = c("season", "week", "team")) %>% 
...
df_pred_total_spread_fin %>% 
  ...
  geom_text(aes(label = game, x = total_game_lower_adj - 1.1), hjust = 0, size = 3.15) +
