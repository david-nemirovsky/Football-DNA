  ...
  left_join(
    df_roster_wr_all %>% 
      ...
      mutate(
        ...
        expect_tar_other = sum(prior_tar_share * prior_n_pass, na.rm = T) - prior_tar_share * prior_n_pass
      ) %>% 
      ungroup() %>% 
...
df_fpts_wr_test = 
  ...
  left_join(df_games_missed_all, by = c("player_id", "season")) %>% 
  left_join(df_def_epa_sos, by = c("team", "season")) %>% 
  filter(prior_rank <= 110 | is.na(prior_rank)) %>% 
  mutate(
    expect_tar = prior_tar_share * prior_n_pass,
    expect_ay = prior_ays * prior_n_pass,
    expect_routes = prior_route_partic * prior_n_pass,
...
df_wr_project_fin_adp = 
  ...
  #filter(is.na(avg))
  mutate(
    adp_wr = as.integer(str_extract(pos_rank_adp, "\\d+")),
    rank_edge_wr = adp_wr - wr_rank_svm
  )

market_curve_wr = 
  df_wr_project_fin_adp %>% 
  drop_na(rank_edge_wr) %>% 
  gam(
    pred_fpts ~ s(adp_wr),
    data = .,
    method = "REML"
...
df_wr_project_market = 
  ...
  mutate(
    ...
    market_edge_wr = pred_fpts - market_proj_wr,
    exp_finish_edge = adp_wr - pred_rank_mean
  )
...
df_adp_beat = 
  ...
  left_join(
    df_wr_project_fin_adp %>% 
      select(team, player_display_name, adp_wr),
    by = c("team", "player_display_name")
  ) %>% 
  group_by(player_id) %>% 
  summarise(
    p_adp_beat = mean(fpts_sim_rank < adp_wr)
  )

df_wr_project_market_fin = 
  df_wr_project_market %>% 
  left_join(df_adp_beat, by = "player_id")
...
df_wr_fantasy_table = 
  df_wr_project_market_fin %>% 
  mutate(
    across(c(market_proj_wr, market_edge_wr, pred_rank_mean, exp_finish_edge), ~ round(., 1))
  ) %>% 
  select(player_display_name, team, wr_rank_svm, adp_wr, rank_edge_wr, pred_fpts, market_proj_wr, market_edge_wr, pred_rank_mean, exp_finish_edge, p_adp_beat, pred_fpts_10p, pred_fpts_90p, starts_with("prop_")) %>% 
...
df_fpts_rb_test =
  ...
  select(player_id, player_display_name, team, prior_rush_rate, prior_fpts, prior_n_pass, prior_rb_epa, player_age, draft_number, rb_new_team, expect_rush_other, games_missed_rate, expect_tar, expect_rush, expect_gl_opp, ol_cont_grade, ol_cont_share) %>% 
  # select(-c(prior_qb_epa)) %>% 
  drop_na()
...
df_rb_project_fin_adp =
  ...
  # filter(is.na(adp))
  mutate(
    adp_rb = as.integer(str_extract(pos_rank_adp, "\\d+")),
    rank_edge_rb = adp_rb - rb_rank_svm
  )

market_curve_rb = 
  df_rb_project_fin_adp %>% 
  drop_na(rank_edge_rb) %>% 
  gam(
    pred_fpts ~ s(adp_rb),
    data = .,
    method = "REML"
...
df_rb_project_market = 
  ...
  mutate(
    ...
    market_edge_rb = pred_fpts - market_proj_rb,
    exp_finish_edge = adp_rb - pred_rank_mean
  )
...
df_adp_beat_rb = 
  ...
  left_join(
    df_rb_project_fin_adp %>% 
      select(team, player_display_name, adp_rb),
    by = c("team", "player_display_name")
  ) %>% 
  group_by(player_id) %>% 
  summarise(
    p_adp_beat = mean(fpts_sim_rank < adp_rb)
  )
...
df_rb_fantasy_table = 
  df_rb_project_market_fin %>% 
  mutate(
    across(c(market_proj_rb, market_edge_rb, pred_rank_mean, exp_finish_edge), ~ round(., 1))
  ) %>% 
  select(player_display_name, team, rb_rank_svm, adp_rb, rank_edge_rb, pred_fpts, market_proj_rb, market_edge_rb, pred_rank_mean, exp_finish_edge, p_adp_beat, pred_fpts_10p, pred_fpts_90p, starts_with("prop_")) %>% 
  select(-c(prop_top_1, prop_top_5)) %>% 
  drop_na()
...
df_te_project_fin_adp =
  ...
  # filter(is.na(adp))
  mutate(
    adp_te = as.integer(str_extract(pos_rank_adp, "\\d+")),
    rank_edge_te = adp_te - te_rank_boost
  )

market_curve_te = 
  df_te_project_fin_adp %>% 
  drop_na(rank_edge_te) %>% 
  gam(
    pred_fpts ~ s(adp_te),
    data = .,
    method = "REML"
...
df_te_project_market = 
  ...
  mutate(
    ...
    market_edge_te = pred_fpts - market_proj_te,
    exp_finish_edge = adp_te - pred_rank_mean
  )
...
df_adp_beat_te = 
  ...
  left_join(
    df_te_project_fin_adp %>% 
      select(team, player_display_name, adp_te),
    by = c("team", "player_display_name")
  ) %>% 
  group_by(player_id) %>% 
  summarise(
    p_adp_beat = mean(fpts_sim_rank < adp_te)
  )
...
df_te_fantasy_table =
  df_te_project_market_fin %>% 
  mutate(
    across(c(market_proj_te, market_edge_te, pred_rank_mean, exp_finish_edge), ~ round(., 1))
  ) %>% 
  select(player_display_name, team, te_rank_boost, adp_te, rank_edge_te, pred_fpts, market_proj_te, market_edge_te, pred_rank_mean, exp_finish_edge, p_adp_beat, pred_fpts_10p, pred_fpts_90p, starts_with("prop_")) %>% 
...
df_fpts_qb_test =
  ...
  filter(prior_rank <= 35 | (is.na(prior_rank))) %>% 
...
df_qb_project_fin_adp =
  ...
  # filter(is.na(adp))
  mutate(
    adp_qb = as.integer(str_extract(pos_rank_adp, "\\d+")),
    rank_edge_qb = adp_qb - qb_rank_svm
  )

market_curve_qb = 
  df_qb_project_fin_adp %>% 
  drop_na(rank_edge_qb) %>% 
  gam(
    pred_fpts ~ s(adp_qb),
    data = .,
    method = "REML"
...
df_qb_project_market = 
  ...
  mutate(
    ...
    market_edge_qb = pred_fpts - market_proj_qb,
    exp_finish_edge = adp_qb - pred_rank_mean
  )
...
df_adp_beat_qb = 
  ...
  left_join(
    df_qb_project_fin_adp %>% 
      select(team, player_display_name, adp_qb),
    by = c("team", "player_display_name")
  ) %>% 
  group_by(player_id) %>% 
  summarise(
    p_adp_beat = mean(fpts_sim_rank < adp_qb)
  )
...
df_qb_fantasy_table =
  df_qb_project_market_fin %>% 
  mutate(
    across(c(market_proj_qb, market_edge_qb, pred_rank_mean, exp_finish_edge), ~ round(., 1))
  ) %>% 
  select(player_display_name, team, qb_rank_svm, adp_qb, rank_edge_qb, pred_fpts, market_proj_qb, market_edge_qb, pred_rank_mean, exp_finish_edge, p_adp_beat, pred_fpts_10p, pred_fpts_90p, starts_with("prop_")) %>% 
  select(-c(prop_top_24, prop_below_36)) %>% 
  drop_na()
