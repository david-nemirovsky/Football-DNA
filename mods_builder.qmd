...
df_fpts_wr_rookie_train =
  ...
  # left_join(coach_df, by = c("team", "season")) %>% 
  # left_join(mod_df, by = c("team", "season")) %>% 
  select(player_id, sum_fpts, prior_qb_epa, prior_n_pass, prior_off_pass_epa, prior_rz_eff, prior_rz_trips, prior_fpts_other, player_age, draft_number) %>% 
  select(-c(prior_off_pass_epa, prior_rz_trips, prior_rz_eff)) %>% 
  drop_na() %>% 
  arrange(player_id) %>%
  select(-player_id)
...
# Resamples
set.seed(37564)
res_resid = resamples(list(GLM = mod_glm_wr_rook_resid, ENET = mod_enet_wr_rook_resid, MARS = mod_mars_wr_rook_resid, KNN = mod_knn_wr_rook_resid, BOOST = mod_boost_wr_rook_resid, SVM = mod_svm_wr_rook_resid))
summary(res_resid)
bwplot(res_resid, metric = "RMSE", main = "RMSE for Repeated 5-Fold CV Using Various Models")
```
...
df_fpts_wr_rookie_test = 
  ...
  left_join(df_off_epa, by = c("team", "season")) %>% 
  left_join(df_rz_eff, by = c("team", "season")) %>% 
  select(player_id, full_name, team, prior_qb_epa, prior_n_pass, prior_off_pass_epa, prior_rz_eff, prior_fpts_other, player_age, draft_number) %>% 
  drop_na()
...
df_wr_project_fin =
  ...
  group_by(player_id, team, wr_rank_svm, player_display_name, pred_fpts) %>% 
  summarise(
    across(
