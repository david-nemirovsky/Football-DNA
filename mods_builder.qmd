...
df_all_fantasy_table =
  ...
  select(player_display_name, team, position, rank_vor, adp, rank_edge, draft_value, pred_fpts, pred_replace, vor) %>% 
  mutate(
    across(c(draft_value, pred_fpts, pred_replace, vor), ~ round(., 1)),
    across(c(rank_vor, adp, rank_edge), ~ round(., 0))
  ) %>% 
  labelled::set_variable_labels(
    "player_display_name" = "Player", 
    "team" = "Team", 
