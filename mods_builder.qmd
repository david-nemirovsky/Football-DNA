...
df_fpts_wr_train = 
  ...
  select(player_id, sum_fpts, prior_ays, prior_tar_share, prior_fpts_game, prior_qb_epa, prior_n_pass, prior_off_pass_epa, def_pass_epa_sos, prior_rz_eff, prior_rz_trips, prior_n_routes, prior_route_partic, prior_yprr, prior_rz_tar_share, player_age, draft_number, prior_fpts_other, expect_tar_other, new_qb_new_team, prior_games_missed, games_missed_rate) %>% 
  mutate(
...
df_fpts_qb_rookie_train =
  ...
  # left_join(df_gl_eff, by = c("team", "season")) %>% 
  left_join(df_def_epa_sos, by = c("team", "season")) %>% 
  select(player_id, sum_fpts, prior_off_pass_epa, prior_n_pass, prior_fpts_other, player_age, draft_number, ol_cont_grade_pass, def_pass_epa_sos) %>% 
...
mod_knn_qb_rook_resid = train(mod_var ~ .,
                ...
                method = "knn", 
                preProcess = c("center","scale"),
                tuneGrid = data.frame(k = seq(20, 50, by = 2)), 
                trControl = ctrl)
...
df_fpts_qb_rookie_test =
  ...
  left_join(df_off_epa, by = c("team", "season")) %>% 
  left_join(df_def_epa_sos, by = c("team", "season")) %>% 
  select(full_name, team, prior_fpts_other, draft_number, def_pass_epa_sos) %>% 
  drop_na()
