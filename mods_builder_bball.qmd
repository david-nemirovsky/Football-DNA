...
progressr::with_progress({
  szn_all_nba <- hoopR::load_nba_pbp(seasons = 2009:2025)
})
tictoc::toc()

szn_all_nba %>% #pull(type_text) %>% table(useNA = "ifany")
  head(10000) %>% view

progressr::with_progress({
  rosters_nba <- hoopR::espn_nba_game_rosters(game_id = c(szn_all_nba %>% distinct(game_id) %>% slice(18001) %>% pull()))
})

nba_game_ids = szn_all_nba %>% filter(season == 2024) %>% distinct(game_id) %>% pull()

safe_rosters = possibly(
  ~ hoopR::espn_nba_game_rosters(game_id = .x) %>%
      select(game_id, team_id, athlete_id, full_name),
  otherwise = tibble()
)

rosters_nba = map_dfr(nba_game_ids, safe_rosters)
...
rosters_nba = NULL
for(i in 1:length(nba_game_ids)){
  rosters_nba[[i]] = 
    hoopR::espn_nba_game_rosters(game_id = nba_game_ids[[i]]) %>% 
    select(game_id, team_id, athlete_id, full_name)
}
